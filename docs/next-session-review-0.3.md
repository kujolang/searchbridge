...
- [ ] Provision dedicated low-privilege provider test properties and GitHub environment secrets. **External-account blocked:** the workflow, GitHub environment, and sanitized evidence contract are complete; dedicated external accounts/properties and credential values are not available in the repository session.
