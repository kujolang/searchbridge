...
[[ "$version" =~ ^[0-9]+\.[0-9]+\.[0-9]+$ ]]
[[ "$version" == "1.0.0" ]]
rg -q "version = \"$version\"" kujo.toml sdk/rust/Cargo.toml
rg -q "export VERSION := \"$version\"" src/core.kujo
...
if rg -n --glob '!sdk/rust/target/**' 'searchbridge-0\.4\.0|tags:.*v0\.\*\.\*|version: .0\.4\.0.|version-0\.4\.0' .github integrations README.md sdk src kujo.toml VERSION; then
